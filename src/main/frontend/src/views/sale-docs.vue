...
export default class IncomeDocs extends Vue {
  ...
  payIncomeDoc(docId: number): void {
    ...
    this.$confirm({
      ...
      async onOk() {
        await SaleDocumentService.paySaleDocument({ saleDocId: docId, company_id: self.$store.state.companyModule.company.id });
  ...
  getAllIncomeDocs(): void {
    SaleDocumentService.getAll({ company_id: this.companyId })
      .then((response) => {
        this.docs = response;
      })
      .finally(() => {
